...
### Added
- `IGraphRAGService.LoadIndexAsync(chunks, options)` — reconstructs a `GraphRAGIndex` for the given chunks from the persisted `IGraphStore`, so an index built in one process can be queried and updated (`UpdateIndexAsync`) after a restart. The chunks define the scope: entities extracted from them, relationships between those entities, and the chunks themselves as `GraphRAGIndex.Chunks`. Requires a graph store (throws `InvalidOperationException` otherwise). The loaded index carries the entity graph only — community hierarchy and summaries are empty until persisted community membership round-trips; global search on a loaded index logs a warning and finds nothing. `GraphRAGLoadOptions.LoadRelationships` skips the per-entity relationship reads.
