...
internal static class CommunityIdentity
{
    ...
    /// <summary>The id of the community at <paramref name="level"/> whose members are <paramref name="chunkIds"/>, in any order.</summary>
    public static string For(int level, IEnumerable<string> chunkIds)
        => ChunkStorageId.CreateNameBasedUuid(Namespace, $"level|{level}|{string.Join('\n', chunkIds.Order(StringComparer.Ordinal))}").ToString();
